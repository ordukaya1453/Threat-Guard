...
def api_status():
    return jsonify({"status":"ok", "version":"v6", "apis": external_api_status(), "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
...
def report():
    ...
    content = f"""ThreatGuard Pro Max v6 Analiz Raporu
...
"""
    ...
    response.headers["Content-Disposition"] = "attachment; filename=ThreatGuard_Pro_Max_v6_Rapor.txt"
    return response
...
if __name__ == "__main__":
    ...
    print(f"ThreatGuard Pro Max v6 çalışıyor: http://0.0.0.0:{port}")
