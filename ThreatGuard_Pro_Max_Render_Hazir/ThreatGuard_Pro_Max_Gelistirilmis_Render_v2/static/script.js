        ...
        if(urlBox && urlList){
            ...
            if(data.url_details && data.url_details.length>0){
                ...
                data.url_details.forEach(item=>{
                    const li=document.createElement("li");
                    let sslText="Sertifika kontrolü yapılmadı";
                    if(item.ssl){
                        if(item.ssl.valid){
                            sslText=`SSL geçerli | Bitiş: ${item.ssl.expires} | Sağlayıcı: ${item.ssl.issuer}`;
                        }else{
                            sslText=`SSL doğrulanamadı/geçersiz${item.ssl.error ? " | Hata: "+item.ssl.error : ""}`;
                        }
                    }
                    li.innerText=`${item.domain} | HTTPS: ${item.https ? "Var" : "Yok"} | Resmî alan adı: ${item.official ? "Evet" : "Hayır"} | ${sslText}`;
